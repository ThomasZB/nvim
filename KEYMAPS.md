...
| `y` / `p` | 复制路径 / 粘贴文件 |
| `Tab` | 多选（配合 `m` 批量移动） |
| `I` | 显示 gitignore 掉的文件（`build/` 等） |
| `H` | 显示隐藏文件（`.clangd` 等点开头的） |
| `]g` / `[g` | 跳到下一个 / 上一个有 git 改动的文件 |
| `Z` | 折叠所有目录 |

> `a` 会自动创建中间目录，直接写 `core/src/utils/new/` 可一次建多层。
> 新建位置是光标所在的目录。
> `I` / `H` 是两个独立开关，`.clangd` 这类既隐藏又被 ignore 的文件要两个都开。
> 在 `空格 f` 文件搜索里对应的是 `Option+i` / `Option+h`。
...
| 键 | 作用 |
|---|---|
| `空格 gg` | lazygit（暂存/提交/分支/冲突，全在里面）**需 `brew install lazygit`** |
| `]h` / `[h` | 下一个 / 上一个改动块 |
| `空格 ghp` | 预览当前改动块 |
| `空格 ghs` / `空格 ghr` | 暂存 / 撤销当前改动块 |
| `空格 ghd` | 当前文件与 HEAD 分屏对比 |
| `空格 gs` | git status |
| `空格 gl` | commit 历史（回车看完整 diff） |
| `空格 gf` | **当前文件的修改历史** |
| `空格 gd` | 所有未提交改动的 diff |
| `空格 gb` | 行级 blame |
| `空格 gB` / `空格 gY` | 在 GitHub 打开当前行 / 复制该链接 |
| `空格 uG` | 开关左侧改动标记 |

> 左侧符号列的增删改标记开箱即用，不需要配置。
> 常用组合：`空格 gf` 查文件演变，`空格 ghp` 看自己刚改了啥，`空格 gs` 提交前过一遍。
